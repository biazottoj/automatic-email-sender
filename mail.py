...
class EmailBuilder:
    ...
    def create_emails(self, sender, subject: str):
        ...
        for c in self.contacts:
            ...
            message = self.template.substitute(project=c[0], name=c[1], email=c[2])
            emails.append(Email(sender, c[2], custom_subject, message))
        return emails

    @staticmethod
    def __read_contacts__(csv_file: str) -> List[Tuple[str, str, str]]:
        """
        ...
        if os.path.isfile(csv_file):
            with open(csv_file, mode='r', encoding='utf-8') as contacts:
                for contact in csv.DictReader(contacts):
                    contacts_list.append((contact["project"], contact["author.name"], contact["author.email"]))
